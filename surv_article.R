surv_article <- function(tot_events,path){
...
tot.events<-tot_events#"NA" #tot.events = total no. of events reported. If not reported, then tot.events="NA"
arm.id<-1 #arm indicator
###END FUNCTION INPUTS
#Read in survival times read by digizeit
digizeit <- read_delim(path,";", escape_double = FALSE, col_names = FALSE, 
                       locale = locale(decimal_mark = ","), 
                       trim_ws = TRUE)

# digizeit<- read.table(file, header = FALSE, sep = ";")
t.S <- digizeit$X1#digizeit[,1]
t.S <- c(0,t.S)
S <- digizeit$X2 #digizeit[,2]
S <- c(1,S)
#Read in published numbers at risk, n.risk, at time, t.risk, lower and upper
# indexes for time interval
t.risk<- floor(t.S)
lower<- purrr::map_dbl(t.risk, function(x) min(which(t.S >=x)))
upper<- purrr::map_dbl(c(t.risk[-1],Inf), function(x) max(which(t.S <x)))
n.risk<- ceiling(S*tot.events)
n.int<- length(n.risk)
n.t<- upper[n.int]
...
IPD_input <- structure(list(IPD[,1],IPD[,2]),.Names = c("time", "status"))

surv_fit <- flexsurv::flexsurvreg(
  survival::Surv(time, status) ~ 1,
  dist = "gengamma", #'llogis'
  data = IPD_input)
